...
func run(t) -> void:
	...
	t.check_eq(ThreatRules.pick_target([_c(40.0, false), _c(120.0, true)], 120.0), 1, "a tank exactly at the aggro range still pulls")
	t.check_eq(ThreatRules.pick_target([_c(40.0, false), _c(130.0, true)], 120.0), 0, "a tank just beyond the aggro range does not (enemies only chase within it)")
	...
	t.check_near(ThreatRules.TANK_PULL_MULT, 1.0, "pull multiplier")
	t.done()
